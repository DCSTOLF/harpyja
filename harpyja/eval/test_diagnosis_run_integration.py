"""Spec 0043 — AC5 integration smoke: the committed diagnosis driver
...
T18 — an operator action, not a test).

Lives in ``harpyja/eval/`` so the suite collects it (the 0041/0042
...
_DRIVER_ARCHIVED = (
    _REPO_ROOT
    / "specs" / ".archive" / "0043-diagnosis" / "diagnosis_run" / "run_diagnosis.py"
)
_DRIVER_LIVE = (
    _REPO_ROOT / "specs" / "0043-diagnosis" / "diagnosis_run" / "run_diagnosis.py"
)
...
def test_diagnosis_driver_stops_and_warns_without_live_infra():
    residents = _residents()
    if residents is not None:
        pytest.skip(
            "live stack reachable — the closure run is T18 (an operator "
    ...
    ledger = _DRIVER.parent / "diagnosis_results.json"
